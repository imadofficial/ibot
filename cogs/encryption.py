...
class Encryption(commands.Cog):
    ...
    async def on_ready(self):
        print(f'''[BOT:COG]              encryption.py loaded''')

    @commands.group()
    async def encode(self, ctx):
        """ All encode methods """
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(title="Encode Commands")
            embed.add_field(name="Engine's:", value="`base32`\n`base64`\n`rot13\n`hex`\n`base85`", inline=False)
    ...
    async def decode(self, ctx):
        """ All decode methods """
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(title="Decode Commands")
            embed.add_field(name="Engine's:", value="Soon!", inline=False)
            embed.set_footer(text=f'{ctx.message.author.name}', icon_url=ctx.message.author.avatar_url)
            await ctx.send(embed=embed)
